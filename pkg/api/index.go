...
func (hs *HTTPServer) setIndexViewData(c *models.ReqContext) (*dtos.IndexViewData, error) {
	...
	if hasEditPermissionInFoldersQuery.Result {
		...
		if c.OrgRole == models.ROLE_ADMIN || c.OrgRole == models.ROLE_EDITOR {
			children = append(children, &dtos.NavLink{Text: "文件夹", SubTitle: "创建一个新文件夹来组织您的仪表板", Id: "folder", Icon: "folder-plus", Url: setting.AppSubUrl + "/dashboards/folder/new"})
		}

		children = append(children, &dtos.NavLink{Text: "导入", SubTitle: "从文件或Grafana.com导入仪表板", Id: "import", Icon: "import", Url: setting.AppSubUrl + "/dashboard/import"})
	...
	if c.IsSignedIn {
		...
		profileNode := &dtos.NavLink{
			...
			Children: []*dtos.NavLink{
				...
				{Text: "修改密码", Id: "change-password", Url: setting.AppSubUrl + "/profile/password", Icon: "lock", HideFromMenu: true},
			},
		}
	...
	if setting.AlertingEnabled && (c.OrgRole == models.ROLE_ADMIN || c.OrgRole == models.ROLE_EDITOR) {
		alertChildNavs := []*dtos.NavLink{
			...
			{Text: "通知通道", Id: "channels", Url: setting.AppSubUrl + "/alerting/notifications", Icon: "comment-alt-share"},
		}
	...
	for _, plugin := range enabledPlugins.Apps {
		if plugin.Pinned {
			...
			if len(appLink.Children) > 0 && c.OrgRole == models.ROLE_ADMIN {
				...
				appLink.Children = append(appLink.Children, &dtos.NavLink{Text: "插件配置", Icon: "cog", Url: setting.AppSubUrl + "/plugins/" + plugin.Id + "/"})
			}
